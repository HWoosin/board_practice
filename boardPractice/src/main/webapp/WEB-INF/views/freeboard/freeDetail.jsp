    ...
    window.onload = function () {
                    ...
                    for(let i=0; i<replyList.length; i++) {
                        ...
                        <div class='reply-content'>
                            <div class='reply-group'>
                                ...
                                <small class='left'>` + (replyList[i].updateDate != null ? parseTime(replyList[i].updateDate) + '(수정 됨)' : parseTime(replyList[i].replyDate)) + `</small>
        ...
        */
        ...
        document.getElementById('modalDelBtn').onclick = () =>{
                ...
                2. fetch함수를 이용해서 DELETE 방식으로 reply/{rno}요청
                3. 서버에서는 요청을 받아서 비밀번호를 확인하고, 비밀번호가 맞으면 삭제를 진행하면 된다.
                4. 만약 비밀번호가 틀렸다면, 문자열을 반환해서 '비밀번호가 틀렸습니다.' 경고창 출력
                삭제 완료되면 모달닫고 목록 요청 다시 보내기. (reset의 여부 잘 판단)
                */
            const rno = document.getElementById('modalRno').value;
            const replyPw = document.getElementById('modalPw').value;
            if(replyPw === '') {
            ...
            function parseTime(regDateTime){
                ...
                let time;
                if(gap < 60 * 60 * 24 * 1000){//하루안돼?
                    if(gap < 60*60*1000){//한시간 안돼?
                        time = '방금 전';
                    }
                    else{
